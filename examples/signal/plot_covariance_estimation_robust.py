...
def plot_cov_ellipse(ax, cov, n_std=2.5, **kwargs):
    ...
    """
    pearson = cov[0, 1] / nmp.sqrt(cov[0, 0] * cov[1, 1])
    ell_radius_x = nmp.sqrt(1 + pearson)
    ell_radius_y = nmp.sqrt(1 - pearson)
    ellipse = Ellipse((0, 0), width=ell_radius_x * 2, height=ell_radius_y * 2,
                      facecolor='none', **kwargs)
    scale_x = nmp.sqrt(cov[0, 0]) * n_std
    scale_y = nmp.sqrt(cov[1, 1]) * n_std
...
def plot_cov_estimators(ax, X, estimators):
    ...
    for i, est in enumerate(estimators):
        C = Covariances(estimator=est).transform(X[None, ...])[0]
...
rs = nmp.random.RandomState(2023)

n_channels, n_inliers = 2, 50
C_ref = np.array([[1, 0.6], [0.6, 1.5]])
X = C_ref @ np.randn(n_channels, n_inliers)
...
Xout = mu[:, None] + scale * np.randn(n_channels, n_outliers)
